...
// Build the start/end of a 45-min appointment in local "YYYYMMDDTHHMMSS" format
function rdvTimes(rdv) {
  ...
  const e = new Date(y, m - 1, d, hh, mm + 45);
...
function gcalLink(rdv, salon) {
  const { start, end } = rdvTimes(rdv);
  const text = encodeURIComponent(`${rdv.prestation} — ${salon?.nom || 'Barbier'}`);
  const details = encodeURIComponent(`Rendez-vous ${rdv.prestation} avec ${rdv.barbier}${rdv.prix ? ` · ${rdv.prix} €` : ''}`);
  const loc = encodeURIComponent(salon ? `${salon.adresse || ''}, ${salon.ville || ''}` : '');
  return `https://www.google.com/calendar/render?action=TEMPLATE&text=${text}&dates=${start}/${end}&details=${details}&location=${loc}`;
}
// Download an .ics file (Apple Calendar, Outlook…)
function downloadICS(rdv, salon) {
  const { start, end } = rdvTimes(rdv);
  const now = new Date();
  ...
  const loc = salon ? `${salon.nom}, ${salon.adresse || ''}, ${salon.ville || ''}` : '';
  const ics = [
    'BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//BarberLink//FR', 'CALSCALE:GREGORIAN',
    'BEGIN:VEVENT', `UID:${rdv.id}@barberlink`, `DTSTAMP:${stamp}`,
    `DTSTART:${start}`, `DTEND:${end}`,
    `SUMMARY:${rdv.prestation} — ${salon ? salon.nom : 'Barbier'}`,
    `DESCRIPTION:Rendez-vous ${rdv.prestation} avec ${rdv.barbier}${rdv.prix ? ' - ' + rdv.prix + ' EUR' : ''}`,
    `LOCATION:${loc}`,
    'BEGIN:VALARM', 'TRIGGER:-PT2H', 'ACTION:DISPLAY', 'DESCRIPTION:Rappel RDV barbier', 'END:VALARM',
    'END:VEVENT', 'END:VCALENDAR',
  ].join('\r\n');
  const blob = new Blob([ics], { type: 'text/calendar;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `rdv-${rdv.date}.ics`;
...
// Reusable "add to calendar" control (Google Agenda + .ics download)
function AddToCalendar({ rdv, salon, compact }) {
  const [open, setOpen] = useState(false);
  return (
    <div style={{ position: 'relative', display: 'inline-block' }}>
      <button className={`btn btn-o ${compact ? 'btn-sm' : ''}`} onClick={() => setOpen(o => !o)}>
        <Icon name="calendar" size={compact ? 13 : 15} /> Ajouter à mon agenda
      </button>
      {open && (
        <div style={{ position: 'absolute', zIndex: 20, top: '110%', left: 0, background: 'var(--card2)', border: '1px solid var(--border)', borderRadius: 10, padding: 6, minWidth: 190, boxShadow: '0 12px 40px rgba(0,0,0,.5)' }}>
          <a href={gcalLink(rdv, salon)} target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)}
            style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 11px', borderRadius: 7, color: 'var(--text)', fontSize: 14, textDecoration: 'none' }}
            onMouseEnter={e => e.currentTarget.style.background = 'var(--surface2)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            <Icon name="calendar" size={15} /> Google Agenda
          </a>
          <button onClick={() => { downloadICS(rdv, salon); setOpen(false); }}
            style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 11px', borderRadius: 7, color: 'var(--text)', fontSize: 14, background: 'none', border: 'none', width: '100%', textAlign: 'left' }}
            onMouseEnter={e => e.currentTarget.style.background = 'var(--surface2)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            <Icon name="clock" size={15} /> Apple / Outlook (.ics)
          </button>
        </div>
      )}
    </div>
...
function QuickBook({ salon, preselectedBarbier, client, onConfirmed }) {
  ...
  if (confirmed) return (
    <div style={{ background: '#3D7A5420', border: '1px solid var(--success)', borderRadius: 10, padding: 20, textAlign: 'center' }}>
      ...
      <p style={{ color: 'var(--text-dim)', marginBottom: 14 }}>{fmtDate(confirmed.date)} à {confirmed.heure} · {confirmed.prix} €{confirmed.promo ? ` (−${confirmed.promo} %)` : ''}</p>
      <AddToCalendar rdv={confirmed} salon={salon} compact />
    </div>
  );
    ...
    <div style={{ maxWidth: 640, margin: '0 auto', padding: '24px 16px', display: 'flex', flexDirection: 'column', height: 'calc(100vh - 54px)' }}>
      ...
      {rdvConfirmed && (
        <div className="afu" style={{ background: '#3D7A5420', border: '1px solid var(--success)', borderRadius: 8, padding: 16, marginBottom: 14 }}>
          ...
          <AddToCalendar rdv={rdvConfirmed} salon={sget(`salon:${rdvConfirmed.salonId}`)} compact />
        </div>
      )}
      ...
      {upcomingReminders.length > 0 && (
        <div className="afu" style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px', marginBottom: 16 }}>
          ...
          {upcomingReminders.map(r => {
            ...
            return (
              <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginTop: 4 }}>
                ...
                <AddToCalendar rdv={r} salon={s} compact />
              </div>
            );
          })}
        </div>
            ...
            <div key={r.id} className="afu card-hover" style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, padding: 14, animationDelay: `${i * 0.07}s` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                  ...
                  {r.date >= today() && r.statut !== 'annulé' && <AddToCalendar rdv={r} salon={sObj} compact />}
