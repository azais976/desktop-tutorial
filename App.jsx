          ...
          {Object.values(FORFAITS).map(f => (
            <div key={f.id} onClick={() => upd('forfait', f.id)}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontFamily: "'Fraunces',serif", fontSize: 20, color: f.color, fontWeight: 600 }}>{f.nom}</span>
    ...
    <div style={{ maxWidth: 760, margin: '0 auto', paddingBottom: 90 }}>
      ...
      <div style={{ padding: '20px 18px 0' }}>
        ...
        <div className="afu s2" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
          ...
          {salon.horaires && <span className="info-pill"><Icon name="clock" size={14} /> {salon.horaires}</span>}
...
function ClientAccount({ client, onUpdateClient, onBack, onRebook }) {
  ...
  const tabs = [{ id: 'rdv', label: <L icon="list" size={15}>Mes RDV</L> }, { id: 'favoris', label: <L icon="heart" size={15}>Favoris</L> }, { id: 'attente', label: <L icon="hourglass" size={15}>Attente</L> }];
...
function SalonDashboard({ salon: initialSalon, onLogout }) {
  ...
  const allTabs = [
    ...
    { id: 'tarifs', label: <L icon="tag" size={15}>Tarifs</L> },
