...
// LANDING
// ============================================================
function LandingScreen({ onClient, onSalon }) {
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, textAlign: 'center' }}>
      <div className="afu" style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginBottom: 20 }}>
          {[0,1,2].map(i => <div key={i} className="pole" style={{ height: 80, animationDelay: `${i * 0.3}s` }} />)}
        </div>
        <h1 style={{ fontSize: 'clamp(2.2rem,6vw,3.5rem)', fontWeight: 900, letterSpacing: '.03em' }}>BarberLink</h1>
        <p style={{ color: 'var(--cream-dim)', fontSize: 20, fontStyle: 'italic', marginTop: 6 }}>La plateforme des barbiers de La Réunion</p>
      </div>

      <div className="divider" style={{ width: 200, margin: '24px auto' }} />

      <p className="afu s1" style={{ color: 'var(--text-dim)', maxWidth: 480, marginBottom: 40, fontSize: 18 }}>
        Trouvez votre barbier idéal · Réservez en 3 clics · Gérez votre salon avec l'IA
      </p>

      <div className="afu s2" style={{ display: 'flex', gap: 20, flexWrap: 'wrap', justifyContent: 'center' }}>
        <button onClick={onClient} className="btn btn-b btn-lg" style={{ minWidth: 200 }}>
          ✂ Je suis client
        </button>
        <button onClick={onSalon} className="btn btn-o btn-lg" style={{ minWidth: 200 }}>
          🏪 Je suis un salon
        </button>
      </div>

      <div className="afu s3" style={{ marginTop: 60, display: 'flex', gap: 32, flexWrap: 'wrap', justifyContent: 'center', color: 'var(--text-dim)', fontSize: 15 }}>
        {['12 salons inscrits','180 RDV ce mois','4.8 ★ de moyenne'].map(s => (
          <span key={s} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ color: 'var(--brass)' }}>◆</span> {s}
          </span>
        ))}
      </div>
    </div>
...
// CLIENT REGISTRATION
// ============================================================
function ClientRegistration({ onDone, onBack }) {
  ...
  return (
    <div style={{ maxWidth: 420, margin: '0 auto', padding: 24 }}>
      ...
      <p className="afu s1" style={{ color: 'var(--text-dim)', marginBottom: 28, fontSize: 15 }}>Gratuit · Sans carte bancaire</p>
...
// SALON DIRECTORY
// ============================================================
function SalonDirectory({ client, onSelectSalon, onAccount }) {
  ...
  function getNote(id) {
    const avis = getAllByPrefix('avis:').filter(a => a.salonId === id);
    return avgNote(avis);
  }

  function hasDispo(s) {
    const rdvs = getAllByPrefix('rdv:').filter(r => r.salonId === s.id && r.date === today() && r.statut !== 'annulé');
    const booked = rdvs.map(r => r.heure);
  ...
  list = list.sort((a, b) => {
    ...
    if (tri === 'prix') {
      const prixMin = s => Math.min(...(s.barbiers || []).flatMap(b => Object.values(b.tarifs || {})).filter(Boolean));
      return prixMin(a) - prixMin(b);
    }
    return 0;
  });
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '24px 16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
        <div>
          ...
          <p style={{ color: 'var(--text-dim)', fontSize: 15 }}>Bonjour {client.nom} — {list.length} salon{list.length !== 1 ? 's' : ''} trouvé{list.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', marginBottom: 20, color: 'var(--text-dim)', fontSize: 15 }}>
        <input type="checkbox" checked={dispoAujourd} onChange={e => setDispoAujourd(e.target.checked)} style={{ accentColor: 'var(--brass)', width: 16, height: 16 }} />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {list.map((s, i) => <SalonCard key={s.id} salon={s} note={getNote(s.id)} onSelect={() => onSelectSalon(s)} index={i} />)}
      </div>
    </div>
  );
}

function SalonCard({ salon, note, onSelect, index }) {
  ...
  return (
    <div className={`card-hover afu s${Math.min(index + 1, 5)}`} onClick={onSelect}
      style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, padding: 16, cursor: 'pointer', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
      <div style={{ width: 52, height: 52, background: 'var(--surface2)', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 24, border: '1px solid var(--border)' }}>✂</div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
          <h3 style={{ fontSize: 17, fontWeight: 700 }}>{salon.nom}</h3>
          {salon.miseEnAvant && <span className="badge-sponsored">Sponsorisé</span>}
        </div>
        <p style={{ color: 'var(--text-dim)', fontSize: 14 }}>{salon.ville} · {salon.adresse}</p>
        <div style={{ display: 'flex', gap: 14, marginTop: 6, flexWrap: 'wrap', fontSize: 14, alignItems: 'center' }}>
          {note > 0 && <span><StarRow n={note} size={13} /> <span style={{ color: 'var(--text-dim)' }}>{note}</span></span>}
          {prixMin && isFinite(prixMin) && <span style={{ color: 'var(--cream-dim)' }}>À partir de {prixMin} €</span>}
          <span style={{ color: 'var(--text-muted)' }}>{salon.barbiers?.length || 0} barbier{salon.barbiers?.length !== 1 ? 's' : ''}</span>
        </div>
      </div>
      <span style={{ color: 'var(--brass)', fontSize: 22, flexShrink: 0 }}>›</span>
    </div>
  );
}
...
function SalonDetail({ salon: initialSalon, client, onBack, onBook, onUpdateClient }) {
  ...
  const tabs = [{ id: 'barbiers', label: '✂ Barbiers & Tarifs' }, { id: 'portfolio', label: '📸 Réalisations' }, { id: 'avis', label: `⭐ Avis (${avis.length})` }];
...
export default function App() {
  ...
  const [bookingTarget, setBookingTarget] = useState(null); // { salon, barbier }
